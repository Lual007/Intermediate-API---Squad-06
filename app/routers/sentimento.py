...
from ..services import services_sentimento
...
def get_sentimentos(db: Session = Depends(get_db)):
    ...
    """
    return services_sentimento.get_sentimentos(db)
...
def sentimentos_recorrentes(db: Session = Depends(get_db)):
    ...
    """
    return services_sentimento.sentimentos_recorrentes(db)
...
def get_sentimento_by_tecnico(id: int, db: Session = Depends(get_db)):
    ...
    """
    return services_sentimento.get_sentimentos_por_id(id,db)
...
def get_atendimento(db: Session = Depends(get_db)):
    ...
    """
    return services_sentimento.get_atendimento(db)
...
def get_tecnico(id: int, db: Session = Depends(get_db)):
    ...
    """
    tecnico = services_sentimento.get_tecnico(id,db)
    if not tecnico:
...
def get_cliente(id: int, db: Session = Depends(get_db)):
    ...
    """
    cliente = services_sentimento.get_cliente(id, db)
    if not cliente:
